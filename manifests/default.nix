...
let
  ...
  purs-bin =
    ...
    builtins.foldl' (
      ...
      in
      if entry != { } then acc // { ${name} = mkPursDerivation ({ inherit version; } // entry); } else acc
  ...
  spago-bin =
    ...
    builtins.foldl' (
      ...
      if legacyEntry == { } && !(builtins.hasAttr "url" entry) && !(builtins.hasAttr "tarball" entry) then
        acc
      else if legacyEntry != { } then
        acc // { ${name} = mkLegacySpagoDerivation ({ inherit version; } // legacyEntry); }
      else
        acc // { ${name} = mkSpagoDerivation version entry; }
    ) { } (builtins.attrNames entries);
  ...
  named =
    ...
    builtins.foldl' (
      ...
      // {
        ${name} =
          all.${group}.${value} or (stdenv.mkDerivation {
